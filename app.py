...
with st.form("audit_form"):
    c1, c2 = st.columns(2)
    with c1: loc = st.selectbox("Ad Space", ["Main St Billboard", "Downtown Bus Stop", "Radio Spot (Morning)", "Instagram Local Feed"])
    with c2: price = st.number_input("Vendor Quote ($)", min_value=0.0, value=510.0)
    submitted = st.form_submit_button("Run Audit")

if submitted:
    if not key_input:
        st.error("Please enter 'DEMO' or a real API key in the sidebar.")
    else:
